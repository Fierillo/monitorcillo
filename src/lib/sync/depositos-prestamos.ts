...
import { fetchMorosidadRaw } from './morosidad';
...
export async function fetchDepositosPrestamosRaw(): Promise<DepositosPrestamosRawRow[]> {
    ...
    const [depositosPesos, depositosUsd, prestamosPesos, prestamosUsd, depositosPublicosPesos, depositosPublicosUsd, prestamosPublicosPesos, prestamosPublicosUsd, tc, pbiAnchors, emae, ipc, morosidad] = await Promise.all([
    ...
    const morosidadByDate = new Map(morosidad.map(row => [row.fecha, row]));
    const months = [...new Set([...series.flatMap(values => [...values.keys()]), ...morosidad.map(row => row.fecha.slice(0, 7))])].sort();
...
export async function ensureDepositosPrestamosTables(): Promise<void> {
    ...
    for (const column of ['mora_irregular_pct', 'mora_incobrable_pct', 'mora_irregular_total_pct', 'mora_familias_pct', 'mora_empresas_pct']) await sql.query(`ALTER TABLE depositos_prestamos_raw ADD COLUMN IF NOT EXISTS ${column} DECIMAL`, []);
    for (const { rawKey } of PNFC_SERIES) await sql.query(`ALTER TABLE depositos_prestamos_raw ADD COLUMN IF NOT EXISTS ${rawKey} DECIMAL`, []);
    ...
    for (const column of ['mora_irregular_pct', 'mora_incobrable_pct', 'mora_total_irregular_pct', 'mora_familias_pct', 'mora_empresas_pct']) await sql.query(`ALTER TABLE depositos_prestamos_normalized ADD COLUMN IF NOT EXISTS ${column} DECIMAL`, []);
    for (const { dbKey } of PNFC_SERIES) await sql.query(`ALTER TABLE depositos_prestamos_normalized ADD COLUMN IF NOT EXISTS ${dbKey} DECIMAL`, []);
