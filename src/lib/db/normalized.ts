...
const NORMALIZED_KEYS: Record<IndicatorType, string[]> = {
    ...
    pobreza: ['fecha', 'pobreza_indec', 'pobreza_utdt_proyectada', 'pobreza', 'preliminar'],
...
function valuesForRow(type: IndicatorType, dataRow: NormalizedDataRow): DbValue[] | null {
    ...
    if (type === 'pobreza') return [fecha, toNullableNumber(row.pobreza_indec), toNullableNumber(row.pobreza_utdt_proyectada), toNullableNumber(row.pobreza), Boolean(row.preliminar)];
