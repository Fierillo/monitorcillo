...
export const CATALOG_INDICATOR_SPECS: Record<string, CatalogIndicatorSpec> = {
    ...
    'depositos-prestamos': {
        type: 'depositos-prestamos',
        referenceLabel: 'Mes anterior',
        betterWhen: 'higher',
        getReferenceDate: date => addMonths(date, -1),
        selectReferenceValue: row => row.prestamosTotalPbi,
        datePrecision: 'month',
        normalizedValueColumn: 'prestamos_total_pbi',
        selectValue: row => row.prestamosTotalPbi,
        rawDateFields: ['depositos_pesos', 'depositos_usd', 'prestamos_pesos', 'prestamos_usd', 'depositos_publicos_pesos', 'depositos_publicos_usd', 'prestamos_publicos_pesos', 'prestamos_publicos_usd'],
        formatValue: value => `Préstamos: ${formatPbiPercentage(value)}`,
        getNextExpectedDate: date => addMonthsFromDate(date, 1),
        getNextExpectedEvents: ({ rawRows, rawDate, today }) => futureMonthly(latestDateWithValue(rawRows, ['depositos_pesos', 'depositos_usd', 'prestamos_pesos', 'prestamos_usd', 'depositos_publicos_pesos', 'depositos_publicos_usd', 'prestamos_publicos_pesos', 'prestamos_publicos_usd']) ?? rawDate, today, 'BCRA'),
    },
    emision: {
        type: 'emision',
    ...
    inflacion: {
        ...
        fallbackValueColumns: ['ipc_equilibra', 'rem', 'ipc_indec', 'ipc_nucleo_indec'],
        selectValue: row => row.ipc ?? row.ipc_equilibra ?? row.rem ?? row.ipc_indec,
        rawDateFields: ['ipc_indec_general', 'ipc_indec_nucleo', 'ipc_equilibra', 'rem'],
        formatValue: formatPercentage,
        getNextExpectedDate: date => addMonthsFromDate(date, 1),
        getNextExpectedEvents: ({ rawRows, rawDate, publicationDate, sourcePublicationDates, today }) => [
            ...
            ...futureMonthly(sourcePublicationDates?.['inflacion-rem'] ?? latestDateWithValue(rawRows, ['rem']), today, 'REM', 15),
        ],
    },
    icg: {
        type: 'icg',
    ...
    sipa: {
        ...
        normalizedValueColumn: 'total',
        selectValue: row => row.total,
        rawDateFields: ['total', 'privado', 'publico', 'casas_particulares', 'autonomos', 'monotributo', 'monotributo_social'],
        formatValue: value => `${formatDecimal(value / 1000)} millones`,
        getNextExpectedDate: date => addMonthsFromDate(date, 1),
        getNextExpectedEvents: ({ rawRows, rawDate, publicationDate, today }) => futureMonthly(publicationDate ?? latestDateWithValue(rawRows, ['total']) ?? rawDate, today, 'SIPA'),
    },
    'balanza-comercial': {
        type: 'balanza',
        referenceLabel: 'Mes anterior',
