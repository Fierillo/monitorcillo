...
export default async function IndicatorDetailPage({ params }: IndicatorPageProps) {
    ...
    if (!config) return <IndicatorCompositeView title={indicator.indicador} subtitle={`Fuente: ${indicator.fuente} | Dato: ${indicator.dato}`} chartTitle={`Evolución de ${indicator.indicador}`} data={[{ fecha: '2024-01', valor: 0 }]} areas={[{ key: 'valor', name: indicator.indicador, color: '#FFD700', type: 'line' }]} methodology={[{ title: indicator.indicador, description: 'Datos históricos pendientes de integración.' }]} indicatorId={indicator.id} />;

    return <IndicatorCompositeView title={indicator.indicador} subtitle={config.subtitle} chartTitle={config.chartTitle} data={config.data} areas={config.areas} methodology={config.methodology} valueFormat={config.valueFormat} yAxisDecimals={config.yAxisDecimals} yAxisLabel={config.yAxisLabel} secondaryYAxis={config.secondaryYAxis} leftYAxisDomain={config.leftYAxisDomain} showTooltipTotal={config.showTooltipTotal} indicatorId={indicator.id} views={config.views} />;
}
